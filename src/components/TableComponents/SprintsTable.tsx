import { useMemo } from 'react'
import type { Sprint, SprintStatus } from '../../types/sprint'
import { useTableState } from '../../hooks/useTableState'
import { SearchBar } from './SearchBar'
import { FilterControls, type FilterOption } from './FilterControls'
import { Pagination } from './Pagination'

const STATUS_COLORS: Record<SprintStatus, string> = {
  'planning': 'bg-slate-500/20 text-slate-300',
  'active': 'bg-blue-500/20 text-blue-300',
  'completed': 'bg-green-500/20 text-green-300',
}

export interface SprintsTableProps {
  sprints: Sprint[]
  isLoading?: boolean
}

/**
 * Refactored SprintsTable with URL-driven state via useTableState.
 * Features:
 * - Search by sprint name
 * - Filter by status (planning/active/completed)
 * - Sort by name, start_date, completion_rate
 * - Pagination with configurable page size
 * - Full browser back/forward support
 */
export function SprintsTable({ sprints, isLoading = false }: SprintsTableProps) {
  const tableState = useTableState({
    page: 1,
    pageSize: 10,
  })

  // Compute filter options
  const statusOptions = useMemo<FilterOption[]>(() => [
    { value: 'planning', label: 'Planning' },
    { value: 'active', label: 'Active' },
    { value: 'completed', label: 'Completed' },
  ], [])

  // Apply filtering and sorting
  const filteredAndSortedSprints = useMemo(() => {
    let result = [...sprints]

    // Apply status filter
    const statusFilter = tableState.state.filters?.status
    if (statusFilter) {
      result = result.filter((sprint) => sprint.status === statusFilter)
    }

    // Apply search across name
    const searchQuery = tableState.state.filters?.search
    if (searchQuery) {
      const query = searchQuery.toLowerCase()
      result = result.filter((sprint) => sprint.name.toLowerCase().includes(query))
    }

    // Apply sorting
    if (tableState.sorting.length > 0) {
      const sort = tableState.sorting[0]
      result.sort((a, b) => {
        let aVal: any = a[sort.id as keyof Sprint]
        let bVal: any = b[sort.id as keyof Sprint]

        // Handle undefined/null values
        if (aVal == null && bVal == null) return 0
        if (aVal == null) return sort.desc ? -1 : 1
        if (bVal == null) return sort.desc ? 1 : -1

        // Handle string comparison
        if (typeof aVal === 'string' && typeof bVal === 'string') {
          const comparison = aVal.localeCompare(bVal)
          return sort.desc ? -comparison : comparison
        }

        // Handle numeric comparison
        if (typeof aVal === 'number' && typeof bVal === 'number') {
          return sort.desc ? bVal - aVal : aVal - bVal
        }

        // Fallback
        if (aVal < bVal) return sort.desc ? 1 : -1
        if (aVal > bVal) return sort.desc ? -1 : 1
        return 0
      })
    }

    return result
  }, [sprints, tableState.state.filters, tableState.sorting])

  // Apply pagination
  const paginatedSprints = useMemo(() => {
    const start = (tableState.page - 1) * tableState.pageSize
    const end = start + tableState.pageSize
    return filteredAndSortedSprints.slice(start, end)
  }, [filteredAndSortedSprints, tableState.page, tableState.pageSize])

  const handleSearchChange = (value: string) => {
    tableState.setColumnFilters([
      {
        id: 'search',
        value,
      },
    ])
  }

  const handleStatusFilterChange = (key: string, value: string | undefined) => {
    tableState.setColumnFilters([
      {
        id: 'status',
        value: value || '',
      },
    ])
  }

  const handleSort = (columnId: string) => {
    tableState.setSorting((prev) => {
      const existing = prev.find((s) => s.id === columnId)
      if (existing) {
        return [{ id: columnId, desc: !existing.desc }]
      }
      return [{ id: columnId, desc: false }]
    })
  }

  const getCompletionRate = (sprint: Sprint): number => {
    if (sprint.taskCount === 0) return 0
    return Math.round((sprint.completedCount / sprint.taskCount) * 100)
  }

  if (isLoading) {
    return (
      <div className="space-y-4">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="bg-slate-800/50 h-12 rounded animate-pulse" />
        ))}
      </div>
    )
  }

  if (sprints.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-slate-400">No sprints available</p>
      </div>
    )
  }

  return (
    <div className="space-y-4">
      {/* Search and Filters */}
      <div className="space-y-3">
        <SearchBar
          value={tableState.state.filters?.search ?? ''}
          onChange={handleSearchChange}
          placeholder="Search sprints by name..."
        />

        <div className="flex flex-col sm:flex-row gap-3">
          <FilterControls
            filters={tableState.state.filters ?? {}}
            onFilterChange={(key, value) => {
              if (key === 'status') handleStatusFilterChange(key, value)
            }}
            filterConfigs={[{ key: 'status', label: 'Status', options: statusOptions }]}
          />

          {tableState.hasActiveFilters && (
            <button
              onClick={tableState.clearFilters}
              className="px-4 py-2 bg-slate-700 hover:bg-slate-600 border border-slate-600 rounded text-white text-sm transition-colors"
            >
              Clear filters
            </button>
          )}
        </div>
      </div>

      {/* Table */}
      {filteredAndSortedSprints.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-slate-400">No sprints match the current filters</p>
        </div>
      ) : (
        <>
          <div className="border border-slate-700 rounded-lg overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-slate-900 border-b border-slate-700">
                <tr>
                  <th className="px-4 py-3 text-left">
                    <button
                      onClick={() => handleSort('name')}
                      className="text-slate-300 hover:text-white font-medium flex items-center gap-1"
                    >
                      Name
                      {tableState.sorting[0]?.id === 'name' &&
                        (tableState.sorting[0]?.desc ? ' ↓' : ' ↑')}
                    </button>
                  </th>
                  <th className="px-4 py-3 text-left">
                    <button
                      onClick={() => handleSort('status')}
                      className="text-slate-300 hover:text-white font-medium"
                    >
                      Status
                    </button>
                  </th>
                  <th className="px-4 py-3 text-left">
                    <button
                      onClick={() => handleSort('startDate')}
                      className="text-slate-300 hover:text-white font-medium flex items-center gap-1"
                    >
                      Start Date
                      {tableState.sorting[0]?.id === 'startDate' &&
                        (tableState.sorting[0]?.desc ? ' ↓' : ' ↑')}
                    </button>
                  </th>
                  <th className="px-4 py-3 text-left">
                    <button
                      onClick={() => handleSort('taskCount')}
                      className="text-slate-300 hover:text-white font-medium"
                    >
                      Tasks
                    </button>
                  </th>
                  <th className="px-4 py-3 text-left">
                    <button
                      onClick={() => handleSort('estimatedPoints')}
                      className="text-slate-300 hover:text-white font-medium"
                    >
                      Points
                    </button>
                  </th>
                  <th className="px-4 py-3 text-left">
                    <button
                      onClick={() => handleSort('completedCount')}
                      className="text-slate-300 hover:text-white font-medium"
                    >
                      Completion
                    </button>
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800">
                {paginatedSprints.map((sprint) => {
                  const completionRate = getCompletionRate(sprint)
                  return (
                    <tr key={sprint.id} className="hover:bg-slate-800/50 transition-colors">
                      <td className="px-4 py-3">
                        <span className="text-white font-medium">{sprint.name}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`px-2 py-1 rounded text-xs font-medium ${STATUS_COLORS[sprint.status]}`}
                        >
                          {sprint.status.charAt(0).toUpperCase() + sprint.status.slice(1)}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-slate-300 text-xs">
                          {sprint.startDate
                            ? new Date(sprint.startDate).toLocaleDateString()
                            : '—'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-slate-300 text-xs">{sprint.taskCount}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-slate-300 text-xs">{sprint.estimatedPoints}</span>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="w-16 bg-slate-800 rounded-full h-1.5">
                            <div
                              className="bg-blue-500 h-1.5 rounded-full transition-all"
                              style={{ width: `${completionRate}%` }}
                            />
                          </div>
                          <span className="text-slate-300 text-xs w-8 text-right">
                            {completionRate}%
                          </span>
                        </div>
                      </td>
                    </tr>
                  )
                })}
              </tbody>
            </table>
          </div>

          {/* Pagination */}
          <Pagination
            page={tableState.page}
            pageSize={tableState.pageSize}
            totalItems={filteredAndSortedSprints.length}
            onPageChange={tableState.setPage}
            onPageSizeChange={tableState.setPageSize}
          />
        </>
      )}
    </div>
  )
}
